...
def _DR_func(K, Omega, G, opts={}):
    ...
    for p in range(3):
        r, *_ = quad(lambda u: fr(p, u), -1, 1, **opts)
        i, *_ = quad(lambda u: fi(p, u), -1, 1, **opts)
...
def _Omega_of_real_n(n, G, sgn, opts={}):
    ...
    '''
    sgn = np.sign(sgn)
    I = [_I_of_real_n(p, n, G, opts) for p in range(3)]
...
def _dOmega_dn(n, G, sgn, opts={}):
    ...
    sgn = np.sign(sgn)
    I = [_I_of_real_n(p, n, G, opts) for p in range(3)]
    dIdn = [_dI_dn(p, n, G, opts) for p in range(3)]
...
def _n_star(G, opts={}, eps=1e-5):
    ...
    def Delta(n): # discriminant
        I = [_I_of_real_n(p, n, G, opts=opts) for  p in range(3)]
...
def _n_of_real_K(K, G, sgn, opts={}, ns=None, Ks=None, eps=1e-5):
    ...
    if ns == None:
        ...
        nb = -1
    n1 = (nb + n0) * 0.5
    k1 = n1 * _Omega_of_real_n(n1, G, sgn, opts=opts)
    ...
    f = lambda n: _Omega_of_real_n(n, G, sgn*(ns-n), opts=opts) * n - K
...
def _extremalOmega(G, sgn, opts={}, eps=1e-5):
    ...
    dOmega_dn = lambda n: _dOmega_dn(n, G, sgn, opts)   
    if G(1) >= 0: # no crossing, one minimum point on each real branch
        ...
        w1 = _Omega_of_real_n(n1, G, sgn, opts)
        return [n1, n1*w1, w1] # return n, K, and Omega

    # crossing case
    ns = _n_star(G, opts=opts, eps=eps) # critical n where two real branches join
    if ns != None:
        ...
        if abs(val) <= eps*abs(w1): # degenerate extremal points
            ...
            w1 = _Omega_of_real_n(n1, G, sgn, opts)
            w2 = _Omega_of_real_n(n2, G, sgn, opts)
...
def _extremalK(G, opts={}, eps=1e-5):
    ...
    if G(1) >= 0: # no crossing
        return [None, None, None]
    ns = _n_star(G, opts=opts) # critical point where two real branches join
    ...
    if ns > eps: # two extremal points on the plus and minus branches, respectively
        ...
        for sgn in [+1, -1]: # search the plus and minus branches
            dK_dn = lambda n: _dK_dn(n, G, sgn, opts)
            ...
            w = _Omega_of_real_n(n, G, sgn, opts)
            n_list.append(n)
            w_list.append(w)
            k_list.append(n*w)
    else: # search the plus branch only
        dK_dn = lambda n: _dK_dn(n, G, +1, opts)
        if abs(ns) <= eps: # ns is about 0 and is an extremal point
            w2 = _Omega_of_real_n(0, G, 0, opts) # Omega at this point
            ...
            n1 = sol1.root
            w1 = _Omega_of_real_n(n1, G, +1, opts)
            n_list = [n1, 0.]
            w_list = [w1, w2]
            k_list = [n1*w1, 0.]
        else:
            ...
            if val > eps: # no extremal point
                ...
                w_list = [_Omega_of_real_n(n, G, +1, opts) for n in n_list]
...
def _xing(G, opts={}):
    ...
    I0, *_ = quad(G, -1, 1, weight='cauchy', wvar=sol.root, **opts)
    I1, *_ = quad(lambda u: G(u)*u, -1, 1, weight='cauchy', wvar=sol.root, **opts)
    I2, *_ = quad(lambda u: G(u)*u*u, -1, 1, weight='cauchy', wvar=sol.root, **opts)
    D = (I2 - I0)**2 - 4*(I1*I1 - I0*I2)

    # critical points
    k_list = [ 0.5*(I0-I2+sgn*np.sqrt(D)) for sgn in [+1, -1]] # K
    n = 1 / sol.root # refractive index
    w_list = [k/n for k in k_list] # values of Omega
...
def _cplx_K_of_0(G, opts={}):
    ...
    I0, *_ = quad(G, -1, 1, weight='cauchy', wvar=0, **opts)
    I0 += 1j * np.pi * G(0)
    I1, *_ = quad(G, -1, 1, **opts)
    I2, *_ = quad(lambda u: G(u)*u, -1, 1, **opts)
    k_list = []
    D = (I2 - I0)**2 - 4*(I1*I1 - I0*I2)
    for sgn in [+1, -1]:
...
def DR_real(G, maxK=1, minK=-1, num_pts=100, opts={}, shift=True, eps=1e-5):
    ...
    if G(1) >= 0: # no crossing, two real branches
        for sgn in [+1, -1]:
            na = _n_of_real_K(minK, G, sgn, opts=opts, eps=eps)
            nb = _n_of_real_K(maxK, G, sgn, opts=opts, eps=eps)
...
def DR_complexK(G, num_pts=100, opts={}, shift=True, eps=1e-5):
    ...
    if G(1) >= 0: # no crossing, two complex K branches
        for sgn, k0 in zip([+1, -1], _cplx_K_of_0(G, opts=opts)):
            nc, kc, wc = _extremalOmega(G, sgn, opts=opts, eps=eps) # extremal Omega points on the real branch
        ...
        k0 = _cplx_K_of_0(G, opts) # complex K at Omega = 0
        nx, kx, wx = _xing(G, opts) # critical points related to crossing
        br = []
...
def DR_complexOmega(G, num_pts=100, opts={}, shift=True, eps=1e-5):
    ...
    if G(1) < 0: # complex Omega branch exists only for crossing
        nx, kx, wx = _xing(G, opts=opts) # critical points related to crossing
        nc, kc, wc = _extremalK(G, opts=opts, eps=eps) # extremal K points on the real branch
        if nc == None: # two complex Omega branches have joined
            kk = np.linspace(kx[1], kx[0], num_pts)
            ww = np.empty(num_pts, dtype=np.cdouble)
            ww[0] = wx[1]; ww[-1] = wx[0]
            ww[1:-1] = _cplx_Omega(kk[1:-1], G, ww[0]*(1+0.1j), opts=opts)
            dr.append((kk, ww))
